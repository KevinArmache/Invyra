/**
 * CSS du design Origami (page de l'invitation ; l'ouverture a le sien, voir
 * opening.js).
 *
 ...
 * design, qui les déplie ensuite. Sans script (vignettes) ou avec « réduire
 * les animations », l'invitation est complète.
 *
 * Le grain du papier (NOISE) suit le réglage « Grain du papier ».
 */
...
/* Inclinaison 3D réservée aux écrans à souris (classe posée par le script).
   Une transformation 3D permanente autour de la fenêtre découpée fait
   disparaître la photo dans Safari. */
.ori-sheet.is-tiltable {
  transform: perspective(1800px) rotateX(var(--rx, 0deg)) rotateY(var(--ry, 0deg));
  transition: transform .8s var(--ease-out);
}
.ori-sheet.is-tiltable.is-tilting { transition: transform .15s linear; }
...
/* Pas de will-change sur les photos découpées : dans WebKit, un calque
   composité sous un clip-path peut ne pas être peint. */
.ori-window-img { position: absolute; inset: -12% 0; background-size: cover; background-position: center; }
...
.ori-frame-img { position: absolute; inset: -10% 0; background-size: cover; background-position: center; }
