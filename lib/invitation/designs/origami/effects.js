import { FIGURES } from "@/lib/invitation/designs/origami/figures";

/**
 * Effets du design Origami. Exécuté après RSVP_SCRIPT, dans l'iframe isolée :
...
(function () {
  ...
  // Parallaxe des photos derrière leur découpe. Décalage 2D : la photo
  // reste peinte dans son cadre découpé (voir .ori-window-img dans le CSS).
  ...
  if (sheet && window.matchMedia && window.matchMedia('(hover: hover) and (pointer: fine)').matches) {
    sheet.classList.add('is-tiltable');
