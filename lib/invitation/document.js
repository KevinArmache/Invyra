  ...
  if (config?.type === "theme") {
    try {
      ...
      return {
        ...
        // Ouverture propre au thème : elle remplace l'ouverture standard.
...
export function themeToCode(config) {
  ...
  return {
    ...
    opening: normalized.content.opening,
    // Un thème qui a sa propre ouverture la garde une fois passé en code.
    openingCode: rendered.openingCode ?? null,
    music: normalized.music,
  };
}
...
function thumbnailConfig(config) {
  if (config?.type !== "theme") return config;
