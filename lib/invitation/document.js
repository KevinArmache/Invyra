...
/**
 * Construction du document HTML d'une invitation, quelle que soit la sorte
 * de modèle (voir template-config.js) :
 * - modèle code : écrit à la main. `openingCode` (facultatif) remplace
 *   l'écran d'ouverture standard par un écran écrit en HTML/CSS/JS ;
 * - modèle design : généré par un design du registre (designs/).
 *
...
function renderParts(config, event, formattedDate) {
  if (isDesignConfig(config)) {
    try {
      const rendered = renderDesign(config, {
      ...
      return {
        ...
        // Ouverture propre au design : elle remplace l'ouverture standard.
        openingCode: rendered.openingCode ?? null,
        music: normalized.music,
      };
    } catch {
      // Design retiré du registre : on retombe sur le rendu minimal plutôt
...
/**
 * Une vignette ne montre que le haut de l'invitation, à 40 % : inutile d'y
 * charger la galerie et les photos des sections suivantes (plusieurs
 * centaines de Ko chacune, multipliées par le nombre de cartes).
 */
function thumbnailConfig(config) {
  if (!isDesignConfig(config)) return config;
...
/**
 * Scripts d'un modèle code écrits dans son HTML (bibliothèques 3D ou
 * d'animation chargées depuis un CDN, effets…) : une vignette n'en a pas
 * besoin, et chacune les chargerait et les exécuterait.
 */
...
export function buildInvitationDocument({
  ...
  const parts = renderParts(
    readOnly ? thumbnailConfig(config) : config,
    event,
    formattedDate,
  );
  if (readOnly && !isDesignConfig(config)) {
    parts.html = parts.html.replace(SCRIPT_TAGS, "");
  }
  ...
  // Seul le script « statique » du design (compte à rebours…) y tourne.
