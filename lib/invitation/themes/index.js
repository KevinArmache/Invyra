...
import origami from "@/lib/invitation/themes/origami";
 ...
 */
export const THEMES = [eclat, origami, elegance, minimal, festive];
 ...
 * donnent pas de variable : le thème les lit dans `style` au rendu.
 */
...
/**
 * Rend un template à thème en `{ html, css, js, fonts, openingCode }`. `js`
 * contient les effets propres au thème ; le script RSVP est ajouté par
 * l'appelant. `openingCode` est l'écran d'ouverture propre au thème
 * (`theme.opening`), `null` quand le thème garde l'ouverture standard.
 *
...
export function renderTheme(config, { details, eventDate }) {
  ...
  return {
    ...
    openingCode: theme.opening
      ? theme.opening({ style, content, details })
      : null,
  };
}
