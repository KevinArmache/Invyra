...
:root {
  ...
  --press-light: color-mix(in srgb, var(--c-card) 45%, #fff);
...
.ori-crease--v { left: 50%; top: 0; bottom: 0; width: 2px; margin-left: -1px; background: linear-gradient(90deg, var(--press-dark), var(--press-light)); opacity: .55; transform-origin: top; animation: ori-grow-y 1.8s var(--ease-fold) .1s both; }
.ori-crease--h { top: 50%; left: 0; right: 0; height: 2px; margin-top: -1px; background: linear-gradient(180deg, var(--press-dark), var(--press-light)); opacity: .55; transform-origin: left; animation: ori-grow-x 1.8s var(--ease-fold) .3s both; }
...
.ori-panel:nth-child(even)::after {
  background: linear-gradient(180deg, color-mix(in srgb, var(--press-light) 70%, transparent), transparent);
