...
export async function updateUserRole(userId, role) {
  ...
  if (userId === session.userId) throw new Error('Vous ne pouvez pas modifier votre propre rôle')
...
export async function suspendUser(userId, suspend = true) {
  await requireAdmin()
  const session = await getSession()
  if (userId === session.userId) throw new Error('Vous ne pouvez pas vous suspendre vous-même')
...
export async function deleteUserAdmin(userId) {
  await requireAdmin()
  const session = await getSession()
  if (userId === session.userId) throw new Error('Vous ne pouvez pas supprimer votre propre compte')
