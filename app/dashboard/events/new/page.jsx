...
export default function NewEventPage() {
  ...
  const STEPS = [
    ...
    {
      id: 2,
      label: t("portal.events.new.steps.template"),
      icon: LayoutTemplate,
    },
  ...
  async function handleCSVImport(guests) {
    ...
    try {
      for (const g of guests) {
        await addGuest(createdEventId, g)
          .then(() => addedCount++)
          .catch(() => {});
      }
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div>
        <h1 className="text-3xl font-bold text-foreground">
          {t("portal.events.new.title")}
        </h1>
        <p className="text-muted-foreground mt-1">
          {t("portal.events.new.subtitle")}
        </p>
      </div>
      ...
      <div className="flex items-center gap-2">
        {STEPS.map((s, i) => (
          <div key={s.id} className="flex items-center gap-2">
            ...
            {i < STEPS.length - 1 && (
              <div
                className={`h-px w-8 transition-colors ${step > s.id ? "bg-primary" : "bg-border"}`}
              />
            )}
          </div>
        ))}
      ...
      {step === 1 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t("portal.events.new.details_desc")}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <label className="text-sm font-medium mb-1 block">
                {t("portal.events.new.labels.title")}
              </label>
              <Input
                value={form.title}
                onChange={(e) =>
        ...
        <div className="space-y-6">
              ...
              <CardTitle className="flex items-center justify-between">
                ...
                <Button asChild size="sm" variant="outline">
                  <Link href="/dashboard/templates/new" target="_blank">
                    ...
                    {t("portal.events.new.buttons.create_template")}
                  </Link>
                </Button>
              </CardTitle>
              <CardDescription>
              ...
              {templatesLoading ? (
                ...
                <div className="text-center py-12 border border-dashed border-border rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground mb-4">
                    {t("portal.events.new.no_templates")}
                  </p>
          ...
          <div className="flex justify-between">
            ...
            >
              ...
              {t("portal.events.new.buttons.back")}
            </Button>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <Button
                  ...
                  disabled={!guestForm.name || !guestForm.email || guestLoading}
                  className="w-full"
                >
                  {guestLoading
                    ? t("portal.events.new.buttons.adding")
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base">
                  {t("portal.events.new.import_csv")}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <CSVImporter
          ...
          <div>
            <Card className="h-full">
              <CardHeader>
                <CardTitle className="text-base">
                  {t("portal.events.new.guest_list")}
                </CardTitle>
                <CardDescription>
                  {addedGuests.length} {t("portal.events.new.guests_added")}
                </CardDescription>
              </CardHeader>
              <CardContent>
                {addedGuests.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    <p className="text-sm">
                      {t("portal.events.new.no_guests_yet")}
                    </p>
                  </div>
                ) : (
