    ...
    <div className="space-y-6">
      <Card>
        <CardHeader className="pb-3">
          <CardTitle>{t('portal.events.details.overview.stats_title')}</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg flex items-center justify-between">
              <span>{t('portal.events.details.overview.active_template')}</span>
              <Button size="sm" variant="outline" asChild className="gap-1.5 text-xs">
                <Link href={`/dashboard/events/${event.id}/edit`}>
                  <Edit className="w-3.5 h-3.5" />
                  {event.invitationTemplate ? t('portal.events.edit.select_template') : t('portal.events.edit.select_template')}
                </Link>
              </Button>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {event.invitationTemplate ? (
              ...
              <div className="text-center py-6 border border-dashed border-border rounded-lg space-y-2">
                ...
                <p className="text-sm text-muted-foreground">{t('portal.events.edit.select_template')}</p>
                <p className="text-xs text-muted-foreground">{t('portal.events.edit.no_preview')}</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">{t('portal.events.details.tabs.overview')}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {event.description && (
              <div>
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">{t('portal.events.new.labels.description')}</span>
                <p className="text-sm mt-1">{event.description}</p>
              </div>
            )}
            {event.customMessage && (
              <div>
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">{t('common.loading')}</span>
