...
function slotLabel(slot, t) {
  if (slot.kind === "url") {
    if (slot.attribute === "href") return t("portal.themes.visual.link");
    return t("portal.themes.visual.image");
  }
  const tag = slot.tag || "";
  if (/^h[1-6]$/.test(tag)) return t("portal.themes.visual.heading");
  if (tag === "button") return t("portal.themes.visual.button");
  if (tag === "a") return t("portal.themes.visual.link_text");
  if (tag === "li") return t("portal.themes.visual.list_item");
  if (tag === "p") return t("portal.themes.visual.paragraph");
  return t("portal.themes.visual.text");
}
      ...
      source.slots.forEach((slot, index) => {
        ...
        if (!byId.has(id)) {
          const group = {
            id,
            title: isOpening
              ? t("portal.themes.sections.opening")
              : source.kind === "css"
                ? t("portal.themes.visual.backgrounds")
                : slot.group || t("portal.themes.visual.content"),
            items: [],
          };
          byId.set(id, group);
          result.push(group);
      ...
      >
        ...
        {palette.length > 0 && (
          <AccordionItem value="colors">
            <AccordionTrigger>
              {t("portal.themes.visual.colors")}
            </AccordionTrigger>
            <AccordionContent>
              <p className="mb-4 text-xs leading-relaxed text-ink-400">
                {t("portal.themes.visual.colors_hint")}
              </p>
              <div className="grid gap-4 sm:grid-cols-2">
                {palette.map(({ hex, count }) => (
                  <ColorField
                    key={hex}
                    label={`${t("portal.themes.visual.color")} · ${count}×`}
